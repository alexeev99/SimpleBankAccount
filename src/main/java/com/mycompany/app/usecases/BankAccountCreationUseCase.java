...
public class BankAccountCreationUseCase {
    ...
    private static Customer createCustomerFromUserConsole() {
        CustomerUserConsoleInput customerInput =
            BankAccountCreationUserConsole.readCustomer();
        CustomerId customerId = new CustomerId();
        NormalizedString firstName = new NormalizedString(
            customerInput.firstName()
        );
        NormalizedString lastName = new NormalizedString(
            customerInput.lastName()
        );
        NormalizedString cityOfBirth = new NormalizedString(
            customerInput.cityOfBirth()
        );
        LocalDate birthDate = LocalDate.parse(customerInput.birthDate());
        Address address =
            BankAccountCreationUseCase.createCustomerAddressFromUserConsole(
                customerInput
            );
    ...
    private static Address createCustomerAddressFromUserConsole(
        ...
        NormalizedString city = new NormalizedString(
            customerInput.address().city()
        );
        PostalCode postalCode = new PostalCode(
            customerInput.address().postalCode()
        );
        StreetNumber streetNumber = new StreetNumber(
            customerInput.address().streetNumber()
        );
        NormalizedString streetName = new NormalizedString(
            customerInput.address().streetName()
        );
